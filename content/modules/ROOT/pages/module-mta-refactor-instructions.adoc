...
Let's look into the issue at the code level. Open up your VS Code Window by navigating to {xxx} and logging in with password {xxxx}. Your VS Code instance has already been pre-populated with the source code of the application.
...
[TIP]
====
The link:https://access.redhat.com/documentation/en-us/migration_toolkit_for_virtualization/1.0/html/installing_and_using_the_migration_toolkit_for_virtualization/about-mtv_mtv[Migration Toolkit for Virtualization (MTV)^] enables you to migrate virtual machines from VMware vSphere or Red Hat Virtualization to OpenShift Virtualization based on link:https://kubevirt.io[KubeVirt^]. This is an add-on to OpenShift Container Platform, enabling you to run and manage virtual machine workloads alongside container workloads.
====

Access the link:{openshift_console_url}/topology/ns/retail-{openshift_cluster_user_name}?view=graph[Topology view^] of retail-{openshift_cluster_user_name} project in the Developer perspective. You will see the `postgresql-database` virtual machine on OpenShift.
...
----
jdbc.url=jdbc:postgresql://postgresql-database.retail-{openshift_cluster_user_name}.svc.cluster.local:5432/customers
----
...
image::mta-refactor/packages.png[Select packages]
...
image::mta-refactor/sso_login.png[openshift_login]
